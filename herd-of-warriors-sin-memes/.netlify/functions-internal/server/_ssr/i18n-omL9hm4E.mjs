...
//#region node_modules/.nitro/vite/services/ssr/assets/i18n-omL9hm4E.js
...
if (typeof window !== "undefined") currentLang = readInitial();
...
function useLanguage() {
	...
	const [lang, setLangState] = (0, import_react.useState)(currentLang);
	(0, import_react.useEffect)(() => {
		const cb = (l) => setLangState(l);
		listeners.add(cb);
		return () => {
			listeners.delete(cb);
		};
	...
	(0, import_react.useEffect)(() => {
		if (!user) return;
		supabase.from("profiles").select("language").eq("id", user.id).maybeSingle().then(({ data }) => {
			const l = data?.language ?? null;
			if (l && (l === "es" || l === "en") && l !== currentLang) setLanguage(l);
		});
	}, [user]);
	const change = async (l) => {
		setLanguage(l);
		if (user) await supabase.from("profiles").update({ language: l }).eq("id", user.id);
	};
	const t = (key) => dict[lang][key] ?? dict.es[key] ?? key;
	return {
		lang,
		setLang: change,
		t
